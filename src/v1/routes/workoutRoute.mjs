...
const route = Router();
import { getAllWorkouts, getOneWorkout, createNewWorkout, updateOneWorkout, deleteOneWorkout } from "../../controllers/workoutController.mjs";
...
route.post("/",
    ...
    body('equipment')
        .isArray({ min: 1 })
        .withMessage("equipment can't be empty"),
    body('exercises')
        .isArray({ min: 1 })
        .withMessage("exercises can't be empty"),
    body('trainerTips')
        .isArray({ min: 1 })
        .withMessage("trainer tips can't be empty or consist of numbers."),
    createNewWorkout);

route.patch("/:workoutId",
    body('name')
        .isString()
        .isLength({ min: 1 })
...
route.delete("/:workoutId",
    param('workoutId')
        .isInt().withMessage("workoutId should be number value")
        .isEmpty().withMessage("parameter ':workoutId' can not be empty"),
    deleteOneWorkout)
